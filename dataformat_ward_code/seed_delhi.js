...
async function seedDatabase() {
  try {
    console.log("Starting Delhi database seeding...");

    // Read the Delhi seed data
    const seedData = JSON.parse(
      fs.readFileSync(
        path.join(
          __dirname,
          "delhi_seed_converted_with_transliterations_hn_kn.json"
        ),
        "utf8"
      )
    );
    ...
    for (const translation of seedData.designation_translations) {
      await prisma.designation_translations.upsert({
        where: {
          code_language: {
            code: translation.designation_code,
            language: translation.language,
          },
        },
        update: {
    ...
    console.log("Delhi database seeding completed successfully!");
  } catch (error) {
...
seedDatabase()
  .then(() => {
    console.log("Delhi seeding completed successfully");
    process.exit(0);
  })
  .catch((error) => {
    console.error("Delhi seeding failed:", error);
    process.exit(1);
  });
